...
public class INode extends DataBlock {
	...
	public INode(int iNodeNumber, int iNodeTblPointer, SuperBlock superBlock) {
	...
	public String getFileModeAsString() {
        ...
        String fileInfo = "";
        fileInfo += ((fileMode & 0x4000) > 0) ? "d" : "-";  // Directory
        fileInfo += ((fileMode & 0x0100) > 0) ? "r" : "-";  // User Read
        fileInfo += ((fileMode & 0x0080) > 0) ? "w" : "-";  // User Write
        fileInfo += ((fileMode & 0x0040) > 0) ? "x" : "-";  // User Execute
        fileInfo += ((fileMode & 0x0020) > 0) ? "r" : "-";  // Group Read
        fileInfo += ((fileMode & 0x0010) > 0) ? "w" : "-";  // Group Write
        fileInfo += ((fileMode & 0x0008) > 0) ? "x" : "-";  // Group Execute
        fileInfo += ((fileMode & 0x0004) > 0) ? "r" : "-";  // Others Read
        fileInfo += ((fileMode & 0x0002) > 0) ? "w" : "-";  // Others Write
        fileInfo += ((fileMode & 0x0001) > 0) ? "x" : "-";  // Others Execute
